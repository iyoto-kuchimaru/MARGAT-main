...
public class TargetPrizm : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.gameObject.name == "targetSphere" || other.gameObject.name == "movableSphere")
        {
            ...
            if (currentText.text == "X:1, Y:0, Z:1")
            {
                newSphere1.SetActive(true);
            }

            if (currentText.text == "X:7, Y:0, Z:1")
            {
                newSphere2.SetActive(true);
            }

            if (currentText.text == "X:7, Y:0, Z:5")
            {
                newSphere3.SetActive(true);
            }

            if (currentText.text == "X:1, Y:0, Z:5")
            {
                newSphere4.SetActive(true);
            }

            if (currentText.text == "X:1, Y:3, Z:3")
            {
                newSphere5.SetActive(true);
            }

            if (currentText.text == "X:7, Y:3, Z:3")
            {
            ...
            if (isObject1Active && isObject2Active && isObject3Active && isObject4Active && isObject5Active && isObject6Active)
            {
